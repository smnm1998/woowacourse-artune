...
오픈미션을 시작을 앞두고 어떤 프로젝트를 진행해야할지 고민을 많이 했습니다. 결론적으로, 개발이라는 것을 막 시작했을 때 진행했던 프로젝트를 재탄생시키로 했습니다.  
과거 `DeepFace API`를 활용한 표정 기반 콘텐츠 추천 서비스를 개발했습니다. 하지만 당시에는 JSON 구조조차 낯설었고, 환경 변수(`.env`) 관리 미숙으로 API Key가 노출되는 등 프로젝트 관리에 큰 허점을 남겼습니다. 결국 애정했던 기획이었음에도 리팩토링을 기약하며 서랍 속에 넣어둘 수밖에 없었습니다.

하지만 이번 4-5주차에 오픈미션이라는 자유도 높은 미션이 제출되었고, 저는 기회라고 생각했습니다.

### 과거 프로젝트와 이번 프로젝트에서 달라진 점

- **Input 변경**: 하드웨어 제약이 있는 표정 인식 대신, 섬세한 감정 분석이 가능한 **텍스트(LLM)** 기반으로 전환했습니다.
- **Focus 이동**: 영화/도서 등 광범위한 추천 대신, **음악**과 **시각적 위로(이미지)**에 집중하여 깊이 있는 경험을 설계했습니다.
- **Tech Growth**: 과거의 스파게티 코드를 버리고, **NestJS**와 **Monorepo** 아키텍처를 도입하여 견고한 시스템을 구축했습니다.
